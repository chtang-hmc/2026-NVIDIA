...
def run_biased_circuit(
    N: int, angles: np.ndarray, T: float, shots: int, n_steps: int, theta_cutoff: float
...
def bf_dcqo_sampler(
    ...
    for iteration in range(n_iter):
        ...
        bitstrings, energies = run_biased_circuit(
            N, angles, T=T, n_steps=n_steps, shots=n_shots, theta_cutoff=theta_cutoff
        )
...
def quantum_enhanced_mts(
    ...
    *,
    ...
    results["solution"] = {"bitstring": best_s, "energy": int(best_E)}
